...
public class Forms {
    /**
     * Calls the appropriate create method for the IModel type passed
     */
    public static Form createForm(IModel model) {
        return switch (model) {
            case SemesterHours semesterHours -> createSemesterHoursForm(semesterHours);
            case SemesterTimeSlot semesterTimeSlot -> createSemesterTimeSlotForm(semesterTimeSlot);
            case Course course -> createCourseForm(course);
            case Schedule schedule -> createScheduleForm(schedule);
            case null, default -> throw new RuntimeException("Unknown model type passed to Forms.createForm");
        };
    }
